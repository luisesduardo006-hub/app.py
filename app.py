...
def corte():
    ...
    return f'''{CSS}<div class="card"><h2>Corte Diario</h2>
        <div style="text-align:center; background:rgba(34,197,94,0.1); padding:20px; border-radius:20px; border:1px solid #22c55e; margin-bottom:15px">
            <small>NETO EN CAJA</small><h1>${t_v - t_p}</h1>
        </div>
        <a href="/exportar_excel" class="btn-nav" style="background:#1d6f42; color:white">📑 REPORTE EXCEL</a>
...
def inventario():
    ...
    for p in prods:
        html += f'<div style="padding:10px; border-bottom:1px solid #333">{p["nombre"]} - ${p["precio"]} <a href="/eliminar_producto/{p["id"]}" style="color:red; float:right">🗑️</a></div>'
