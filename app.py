...
app = Flask(__name__)
app.secret_key = 'sistema_estable_v8_final'
...
<style>
    ...
    .opcion { display: block; color: #0f0; text-decoration: none; margin: 10px 0; padding: 10px; border-bottom: 1px solid #111; }
    ...
    .btn-pagar { background: #25D366; color: #000; padding: 5px 10px; text-decoration: none; font-weight: bold; border-radius: 3px; }
...
def verificar():
    # SOLUCIÓN AL ERROR 405: Aceptamos GET y buscamos la clave en la sesión
    ...
    if user:
        ...
        if datetime.now() > v_dt and user['rango'] != 'Super Admin':
            return f"{CSS}<div class='menu-box'><h2 style='color:red'>🛑 ACCESO BLOQUEADO</h2><p>Cuenta vencida el: {vence_str}</p><a href='/' class='btn-volver'>VOLVER</a></div>"

        if user['estado'] != "Activo":
            return f"{CSS}<div class='menu-box'><h2 style='color:red'>🛑 CUENTA SUSPENDIDA</h2><a href='/' class='btn-volver'>VOLVER</a></div>"

        # Guardar clave en sesión para navegación segura
        ...
        if user['rango'] in ['Super Admin', 'Administrador']:
            ...
            if user['rango'] == 'Dueño':
                ...
                menu += f'<a class="opcion" href="/hacer_corte_final/{clave}" onclick="return confirm(\'¿Hacer corte? Se borrarán las ventas del día.\')">🏁 REALIZAR CORTE</a>'
        
        menu += '<hr><a class="opcion" href="/" style="text-align:center; color:red;">🚪 SALIR</a></div>'
        return menu
    return redirect('/')

# --- FUNCIONES DE CORTE Y EXCEL ---
...
def hacer_corte_final(clave):
    ...
    writer.writerow(['ID_VENTA', 'TOTAL', 'HORA', 'CAJERO'])
    ...
    return f'''{CSS}<div class="menu-box"><h3>📊 CORTE COMPLETADO</h3><p>Ventas Hoy: ${total_v}</p>
    <a href="/descargar/{ruta.replace('/','_')}" class="btn-pagar">📥 DESCARGAR EXCEL</a>
    <a href="/verificar" class="btn-volver">VOLVER AL MENÚ</a></div>'''
...
def gestionar_usuarios(clave):
    ...
    if user['rango'] == "Super Admin":
        ...
        tabla = "<table>"
        for a in admins:
            tabla += f'<tr><td>{a["nombre"]}</td><td>{a["reactivaciones"]}</td><td><a href="/borrar/{clave}/{a["clave"]}" class="btn-rojo">X</a></td></tr>'
        r_crear = "Administrador"
    else:
        trbs = db_u.execute("SELECT * FROM usuarios WHERE creado_por = ?", (clave,)).fetchall()
        tabla = "<table>"
        for t in trbs:
            tabla += f'<tr><td>{t["nombre"]}</td><td>{t["estado"]}</td><td><a href="/status_simple/{clave}/{t["clave"]}" class="btn-rojo">ESTADO</a></td></tr>'
        r_crear = "Trabajador"
...
def add_user():
    ...
    return f"{CSS}<div class='menu-box'><h3>✅ CLAVE GENERADA: {clv}</h3><a href='/' class='btn-volver'>LOGIN</a></div>"
...
def status_cascada(admin, target):
    ...
    with db_u:
        db_u.execute("UPDATE usuarios SET estado=? WHERE clave=?", (new, target))
        db_u.execute("UPDATE usuarios SET estado=? WHERE creado_por=?", (new, target))
    return redirect(f"/gestionar_negocios/{admin}")
...
def config_corte(clave):
    ...
    return f'''{CSS}<div class="menu-box"><h3>⚙️ CONFIGURACIÓN</h3><form method="post">
    Negocio: <input name="nom" value="{conf['nombre_negocio']}">
    WhatsApp: <input name="tel" value="{conf['telefono_dueno']}">
    Hora: <input type="time" name="hora" value="{conf['hora_corte']}">
    <button type="submit">GUARDAR</button></form><a href="/verificar" class="btn-volver">VOLVER</a></div>'''
